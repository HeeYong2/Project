...
HRESULT CTerrain::Initialize(const _uint& wCntX, const _uint& wCntZ, const float& fItv)
{
	...
	m_VtxSizeZ = wCntZ;
	m_fVtxItv = fItv;
 	Engine::Throw_Vertex_Info(RESOURCE_STAGE, L"Buffer_Terrain", m_pVtxInfo);
...
CTerrain* CTerrain::Create(const _uint& wCntX, const _uint& wCntZ, const float& fItv)
{
	CTerrain* pTerrain = new CTerrain;

	if(FAILED(pTerrain->Initialize(wCntX , wCntZ , float(fItv))))
	{
//		safe_delete(pTerrainTool);
		msg_box("pTerrainTool Create Failed");
		return NULL;
	}
	return pTerrain;
}
...
HRESULT CTerrain::Add_Component(void)
{
	...
	//= For.Buffer =======================================================================
	pComponent = m_pBuffer = Engine::Clone_Resource(RESOURCE_STAGE, L"Buffer_Terrain");
	if(NULL == pComponent)
		return E_FAIL;
...
void CTerrain::Render_Texture(void)
{
...
//------------------------------------------------------------------------------------------------------------------------------
void CTerrain::Set_ConstantTable(void)
{
	if(NULL == m_pEffect)
		return;
...
Engine::VTXTEX* CTerrain::GetVtxTex(void)
{
	return m_pVtxInfo;
}

//--------------------------------------------------------------------------------------------------------------------------------------------------------
Engine::CComponent* CTerrain::GetBuffer(void)
{
	return m_pBuffer;
}
//--------------------------------------------------------------------------------------------------------------------------------------------------------

void CTerrain::SetFrameType(_int iFrameNum)
{
...
void CTerrain::SetVerTex(_vec3 vPos)
{
...
WORD CTerrain::GetVtxSizeX(void)
{
	return m_VtxSizeX;
}

WORD CTerrain::GetVtxSizeZ(void)
{
	return m_VtxSizeZ;
}

_float CTerrain::GetVtxItv(void)
{
	return m_fVtxItv;
}

void CTerrain::SetLoadVerTex(_vec3 vPos)
{
	((Engine::CVIBuffer*)m_pBuffer)->SetVtxInfo(m_pVtxInfo);
}

void CTerrain::SetTextureNum(_int iTextureNum)
{
	m_iTextureNum = iTextureNum;
}

void CTerrain::SetUVRatio(_float UVRatio)
{
	m_fUVRatio = UVRatio;
}

void CTerrain::CreateAlphaMap(void)				//버튼을 눌렀을때 알파맵이 생성되게 한다
{
...
LPDIRECT3DTEXTURE9 CTerrain::GetTexture(_int iNum)
{
	return m_pAlphaTexture[iNum];
}

void CTerrain::SetMix1num(_int iMix1Num)
{
	m_tTexInfo.iTextureNum[0] = iMix1Num;
}

void CTerrain::SetMix2num(_int iMix2Num)
{
	m_tTexInfo.iTextureNum[1] = iMix2Num;
}

void CTerrain::SetMix3num(_int iMix3Num)
{
	m_tTexInfo.iTextureNum[2] = iMix3Num;
}

void CTerrain::SetMix4num(_int iMix4Num)
{
	m_tTexInfo.iTextureNum[3] = iMix4Num;
}

void CTerrain::SetMix1Ratio(_float fMix1Ratio)
{
	m_tTexInfo.fAlpahUV[0] = fMix1Ratio;
}

void CTerrain::SetMix2Ratio(_float fMix2Ratio)
{
	m_tTexInfo.fAlpahUV[1] = fMix2Ratio;
}

void CTerrain::SetMix3Ratio(_float fMix3Ratio)
{
	m_tTexInfo.fAlpahUV[2] = fMix3Ratio;
}

void CTerrain::SetMix4Ratio(_float fMix4Ratio)
{
	m_tTexInfo.fAlpahUV[3] = fMix4Ratio;
}

void CTerrain::SetAlphaTexture(LPDIRECT3DTEXTURE9* ppLoadTexture)
{
	for(int i = 0 ; i < 4 ; ++i)
...
TEXINFO CTerrain::GetTexInfo(void)
{
	return m_tTexInfo;
}

void CTerrain::SetTexInfo(TEXINFO tTexInfo)
{
	m_tTexInfo = tTexInfo;
}